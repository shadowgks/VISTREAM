                ...
                <div x-data='{ open: false }'>
                    ...
                    <div class="bg-white p-8 dark:bg-color-primary-75 h-fit">
                        <div class="text-right">
                            <button @click="open = true" type="button" onclick="addGenre()"
                            ...
                            <tbody>
                                @foreach ($genre as $key => $item)
                                    <tr>
                                        ...
                                        <td>{{ $item->name }}</td>
                                        <td>
                                            <button type="button" @click="open = !open" onclick="modalGenreEdit('{{ $item->id }}','{{ $item->name }}')" class="inline-block mr-2 hover:text-green-500" title="Edit">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil-square" viewBox="0 0 16 16">
                                                    <path d="M15.502 1.94a.5.5 0 0 1 0 .706L14.459 3.69l-2-2L13.502.646a.5.5 0 0 1 .707 0l1.293 1.293zm-1.75 2.456-2-2L4.939 9.21a.5.5 0 0 0-.121.196l-.805 2.414a.25.25 0 0 0 .316.316l2.414-.805a.5.5 0 0 0 .196-.12l6.813-6.814z"></path>
                                                    <path fill-rule="evenodd" d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5v11z"></path>
                                                </svg>
                                            </button>
                                            <button type="button" @click="open2 = !open2" onclick="modalGenreTrash('{{ $item->id }}')" <svg xmlns="http://www.w3.org/2000/svg" class="inline-block mr-2  hover:text-red-500" title="Delete">
                    ...
                    <div id="modal_add_genre" x-show="open" tabindex="0" class="z-50 overflow-auto inset-0 w-full h-full fixed py-6">
                        <div class="z-50 relative p-3 mx-auto my-0 w-[40%]" style="min-width: 500px;display: none" x-show="open" x-transition:enter="transition duration-500" x-transition:enter-start="transform opacity-0 -translate-y-4"
                            ...
                            <div class="bg-white rounded shadow-lg border flex flex-col overflow-hidden dark:bg-gray-800 dark:border-gray-700">
                                ...
                                <!-- modal title -->
                                <div class="px-6 py-3 text-xl border-b dark:border-gray-700 font-bold">Modal Genre</div>
                                {{-- actions --}}
                                <input type="hidden" id="route_store_genre" value="{{ route('genre.store') }}">
                                <input type="hidden" id="route_update_genre" value="{{ route('genre.update', '') }}">
                                <form method="POST" action="{{ route('genre.store') }}" name="form_genre" data-parsley-validate>
                                    @csrf
                                    <div id="methode_put_genre">
                    ...
                    {{-- Modal delete --}}
                    <div id="modal_delete_genre" x-show="open2" tabindex="0" class="z-50 overflow-auto inset-0 w-full h-full fixed py-6">
                        <div @click.away="open2 = false" class="z-50 relative p-3 mx-auto my-0 w-[40%]" style="min-width: 500px;display: none" x-show="open2" x-transition:enter="transition duration-500" x-transition:enter-start="transform opacity-0 -translate-y-4"
                            ...
                            <div class="bg-white rounded shadow-lg border flex flex-col overflow-hidden dark:bg-gray-800 dark:border-gray-700">
                                ...
                                <div class="p-6 flex-grow text-center text-2xl">
                                    <p>Are you sure you went deleted this genre!</p>
                                </div>
                                {{-- methode --}}
                                <input type="hidden" id="route_destroy_genre" value="{{ route('genre.destroy', '') }}">
                                <form action="{{ route('genre.store') }}" method="POST" name="form2_genre">
                                    <div class="px-6 py-3 border-t dark:border-gray-700 flex justify-end">
                                        @csrf
                                        @method('DELETE')
