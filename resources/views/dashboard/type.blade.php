                ...
                <div x-data='{ open: false }'>
                    ...
                    <div class="bg-white p-8 dark:bg-color-primary-75 h-fit">
                        <div class="text-right">
                            <button @click="open = true" type="button" onclick="addType()"
                            ...
                            <tbody>
                                @foreach ($type as $key => $item)
                                    <tr>
                                        ...
                                        <td>{{ $item->name }}</td>
                                        <td>
                                            <button type="button" @click="open = !open" onclick="modalTypeEdit('{{ $item->id }}','{{ $item->name }}')"
                                            ...
                                            <button type="button" @click="open2 = !open2" onclick="modalTypeTrash('{{ $item->id }}')"
                    ...
                    <!-- Modal add -->
                    <div id="modal_add_type" x-show="open" tabindex="0" class="z-50 overflow-auto inset-0 w-full h-full fixed py-6">
                        <div @click.away="open = false" class="z-50 relative p-3 mx-auto my-0 w-[40%]" style="min-width: 500px;display: none" x-show="open" x-transition:enter="transition duration-500" x-transition:enter-start="transform opacity-0 -translate-y-4"
                            ...
                            <div class="bg-white rounded shadow-lg border flex flex-col overflow-hidden dark:bg-gray-800 dark:border-gray-700">
                                ...
                                <!-- modal title -->
                                <div class="px-6 py-3 text-xl border-b dark:border-gray-700 font-bold">Modal type</div>
                                {{-- actions --}}
                    ...
                    {{-- Modal delete --}}
                    <div id="modal_delete_type" x-show="open2" tabindex="0" class="z-50 overflow-auto inset-0 w-full h-full fixed py-6">
                        <div @click.away="open2 = false" class="z-50 relative p-3 mx-auto my-0 w-[40%]" style="min-width: 500px;display: none" x-show="open2" x-transition:enter="transition duration-500" x-transition:enter-start="transform opacity-0 -translate-y-4"
                            ...
                            <div class="bg-white rounded shadow-lg border flex flex-col overflow-hidden dark:bg-gray-800 dark:border-gray-700">
                                ...
                                <div class="p-6 flex-grow text-center text-2xl">
                                    <p>Are you sure you went deleted this type!</p>
                                </div>
                                {{-- methode --}}
                                <input type="hidden" id="route_destroy_type" value="{{ route('type.destroy', '') }}">
                                <form action="{{ route('type.store') }}" method="POST" name="form2_type">
                                    <div class="px-6 py-3 border-t dark:border-gray-700 flex justify-end">
                                        @csrf
                                        @method('DELETE')
